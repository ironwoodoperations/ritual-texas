...
// Whitney's number — change here if needed
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ background: "#132336", border: "1px solid rgba(198,168,94,.3)", borderRadius: "16px", padding: "28px 24px", maxWidth: "360px", width: "100%", textAlign: "center" }}>
        ...
        <p style={{ color: "#9AA8B5", fontSize: "14px", fontFamily: "sans-serif", marginBottom: "20px", lineHeight: "1.5" }}>
          Would you like to text Whitney that <strong style={{ color: "#C6A85E" }}>{task.roomNumber}</strong> is finished and ready?
        </p>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <button
            onClick={onClose}
            style={{ flex: 1, padding: "12px", background: "rgba(245,240,232,.06)", border: "1px solid rgba(198,168,94,.2)", borderRadius: "10px", color: "#9AA8B5", cursor: "pointer", fontFamily: "sans-serif", fontSize: "14px" }}
          >
            Skip
          </button>
          <a
            href={smsUrl}
            onClick={onClose}
            style={{ flex: 2, padding: "12px", background: "#4CAF50", border: "none", borderRadius: "10px", color: "#fff", cursor: "pointer", fontWeight: 700, fontFamily: "sans-serif", fontSize: "14px", textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}
          >
...
export default function StaffHousekeepingTask({ taskId, onBack }) {
  ...
  const updateItemMutation = useMutation({
    ...
    onMutate: async ({ item, updates }) => {
      ...
      qc.setQueryData(["hk-task-items", taskId], (old) => (old || []).map((i) => (i.id === item.id ? { ...i, ...updates } : i)));
      return { prev };
    },
    onError: (_e, _v, ctx) => { if (ctx?.prev) qc.setQueryData(["hk-task-items", taskId], ctx.prev); },
    onSuccess: () => { qc.invalidateQueries(["hk-task", taskId]); },
  });
  ...
  const createIssueMutation = useMutation({
    mutationFn: (data) =>
      base44.entities.HkIssue.create({ ...data, roomId: task.roomId, roomNumber: task.roomNumber, taskId, createdByUserId: "housekeeper" }),
    onSuccess: () => {
  ...
  if (!task) {
    return (
      <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#9AA8B5", fontFamily: "sans-serif" }}>Loading...</div>
      </div>
    );
  }
    ...
    <div style={S.page}>
      ...
      <div style={{ background: "linear-gradient(135deg,#0C1C2C 0%,#132336 100%)", borderBottom: "1px solid rgba(198,168,94,.2)", padding: "16px 20px", position: "sticky", top: 0, zIndex: 40 }}>
        <div style={{ maxWidth: "700px", margin: "0 auto", display: "flex", alignItems: "center", gap: "14px" }}>
          <button onClick={onBack} style={{ background: "none", border: "none", color: "#9AA8B5", cursor: "pointer", display: "flex", padding: 0 }}>
            <ArrowLeft size={20} />
          </button>
          <div style={{ flex: 1 }}>
            <div style={{ color: "#F5F0E8", fontWeight: 600, fontSize: "18px" }}>{task.roomNumber}</div>
            <div style={{ color: "#9AA8B5", fontSize: "12px", fontFamily: "sans-serif" }}>{task.taskType?.replace(/_/g, " ")} · {task.taskDate}</div>
          </div>
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto", padding: "16px 20px" }}>
        ...
        {(task.adminNotes || roomNotes.length > 0) && (
          <div style={{ background: "rgba(198,168,94,.08)", border: "1px solid rgba(198,168,94,.3)", borderRadius: "12px", padding: "16px", marginBottom: "16px" }}>
            <p style={{ color: "#C6A85E", fontSize: "11px", letterSpacing: "2px", margin: "0 0 8px", fontFamily: "sans-serif" }}>📌 NOTES & REQUESTS</p>
        ...
        <div style={{ background: "rgba(245,240,232,.04)", border: "1px solid rgba(198,168,94,.15)", borderRadius: "12px", padding: "16px", marginBottom: "12px" }}>
          <p style={{ color: "#C6A85E", fontSize: "11px", letterSpacing: "2px", margin: "0 0 10px", fontFamily: "sans-serif" }}>📝 MY NOTES</p>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#0C1C2C", borderTop: "1px solid rgba(198,168,94,.2)", padding: "14px 20px", display: "flex", gap: "10px", justifyContent: "center" }}>
        <div style={{ maxWidth: "700px", width: "100%", display: "flex", gap: "10px" }}>
          {task.status === "completed" ? (
            ...
            <>
              {task.status === "pending" && (
                <button onClick={handleStart} style={{ flex: 1, padding: "14px", background: "rgba(198,168,94,.1)", border: "1px solid rgba(198,168,94,.3)", borderRadius: "10px", color: "#C6A85E", cursor: "pointer", fontWeight: 700, fontFamily: "sans-serif", fontSize: "15px" }}>
                  Start
                </button>
              )}
              ...
              <button onClick={handleComplete} style={{ flex: 2, padding: "14px", background: "#4CAF50", border: "none", borderRadius: "10px", color: "#fff", cursor: "pointer", fontWeight: 700, fontFamily: "sans-serif", fontSize: "15px" }}>
                ✓ Complete Room
              </button>
            </>
          )}
