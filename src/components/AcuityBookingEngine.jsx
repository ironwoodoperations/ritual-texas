...
export default function AcuityBookingEngine({
  stayDates = [],
  guestNames = [],
  onBookingSelected,
  onBookingComplete,
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        setError(null);
        const res = await base44.functions.invoke('acuityGuestGetServices', {});
        if (cancelled) return;
        if (res.data?.error) {
  ...
  const fetchAvailability = useCallback(async (service, providerId) => {
    ...
    setAvailabilityMap({});
    try {
      const res = await base44.functions.invoke('acuityGuestGetAvailability', {
  ...
  function handleSelectService(svc) {
    ...
    if (!svc.providers || svc.providers.length <= 1) {
      const p = svc.providers?.[0] || null;
      setSelectedProvider(p);
      fetchAvailability(svc, p?.id);
      setStep(STEPS.DATE);
    } else {
  ...
  function handleConfirmBooking() {
    ...
    if (onBookingSelected) onBookingSelected(selection);
    setStep(STEPS.DONE);
  }
  ...
  function handleFinish() {
    if (onBookingComplete) onBookingComplete(completedBookings);
  }
  ...
  const headingStyle = { fontFamily: 'Georgia, serif', fontSize: '22px', color: colors.primary, fontWeight: 400, marginBottom: '16px' };
  const subheadStyle = { fontSize: '13px', color: '#8B7355', marginBottom: '20px' };
  const pillBtnBase = { padding: '10px 18px', borderRadius: '24px', fontSize: '13px', cursor: 'pointer', transition: 'all 0.15s', border: 'none', fontWeight: 500 };
  const primaryBtn = { width: '100%', padding: '13px 20px', fontSize: '15px', fontWeight: 600, color: '#fff', backgroundColor: colors.primary, border: 'none', borderRadius: '10px', cursor: 'pointer' };
  const secondaryBtn = { width: '100%', padding: '13px 20px', fontSize: '15px', fontWeight: 600, color: colors.primary, backgroundColor: 'transparent', border: `2px solid rgba(59,72,49,.15)`, borderRadius: '10px', cursor: 'pointer' };
  const backLink = { fontSize: '13px', color: '#8B7355', cursor: 'pointer', border: 'none', background: 'none', textDecoration: 'underline', padding: 0, marginBottom: '16px', display: 'inline-block' };
  ...
  if (step === STEPS.SERVICE) {
    return (
      <div>
        ...
        <div style={cardStyle}>
          <h2 style={headingStyle}>{completedBookings.length > 0 ? 'Add Another Treatment' : 'Select a Treatment'}</h2>
          <p style={subheadStyle}>Choose from our signature spa experiences.</p>
          {guestNames.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: '#8B7355', marginBottom: '8px' }}>Who is this treatment for?</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {guestNames.map(name => {
                  const active = selectedGuest === name;
                  return (
                    <button key={name} onClick={() => setSelectedGuest(name)} style={{ ...pillBtnBase, backgroundColor: active ? colors.primary : colors.card, color: active ? '#fff' : colors.primary, border: active ? `2px solid ${colors.primary}` : `1px solid rgba(59,72,49,.15)`, fontWeight: active ? 700 : 500 }}>
                      {name}
                    </button>
                  );
                })}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {services.map(svc => (
              <button key={svc.id} onClick={() => handleSelectService(svc)}
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', backgroundColor: colors.card, border: `1px solid rgba(59,72,49,.10)`, borderRadius: '12px', cursor: 'pointer', textAlign: 'left', width: '100%' }}
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: '12px', color: '#8B7355' }}>{svc.duration} min{svc.providers.length > 0 ? ` · ${svc.providers.length} therapist${svc.providers.length > 1 ? 's' : ''}` : ''}</p>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
          {completedBookings.length > 0 ? (
            <button onClick={handleFinish} style={primaryBtn}>Continue with {completedBookings.length} Treatment{completedBookings.length > 1 ? 's' : ''}</button>
          ) : onSkip ? (
            <button onClick={handleSkip} style={secondaryBtn}>Skip Treatments</button>
          ) : null}
        </div>
      </div>
    );
    ...
    return (
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button onClick={() => handleSelectProvider(null)}
            style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 18px', backgroundColor: colors.card, border: `1px solid rgba(59,72,49,.10)`, borderRadius: '12px', cursor: 'pointer', textAlign: 'left', width: '100%' }}
            onMouseOver={e => { e.currentTarget.style.borderColor = colors.accent; }}
            onMouseOut={e => { e.currentTarget.style.borderColor = 'rgba(59,72,49,.10)'; }}
          >
            <div style={{ width: '44px', height: '44px', borderRadius: '50%', backgroundColor: 'rgba(59,72,49,.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', flexShrink: 0 }}>✦</div>
            <div>
          ...
          {selectedService?.providers?.map(p => (
            <button key={p.id} onClick={() => handleSelectProvider(p)}
              ...
              onMouseOut={e => { e.currentTarget.style.borderColor = 'rgba(59,72,49,.10)'; }}
            >
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', backgroundColor: 'rgba(197,124,93,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: 700, color: colors.accent, flexShrink: 0 }}>
  ...
  if (step === STEPS.DATE) {
    return (
      <div style={cardStyle}>
        <button onClick={() => { selectedService?.providers?.length > 1 ? setStep(STEPS.PROVIDER) : (setStep(STEPS.SERVICE), setSelectedService(null)); }} style={backLink}>← Back</button>
        <h2 style={headingStyle}>Pick a Date</h2>
        <p style={subheadStyle}>{selectedService?.name}{selectedProvider ? ` with ${selectedProvider.name}` : ' · any available therapist'}</p>
        {availabilityLoading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '10px' }}>
            {stayDates.map(date => {
              ...
              return (
                <button key={date} onClick={() => hasSlots && handleSelectDate(date)} disabled={!hasSlots}
                  style={{ padding: '14px 12px', borderRadius: '12px', border: hasSlots ? `1px solid rgba(59,72,49,.15)` : `1px solid rgba(0,0,0,.06)`, backgroundColor: hasSlots ? colors.card : 'rgba(0,0,0,.03)', cursor: hasSlots ? 'pointer' : 'not-allowed', opacity: hasSlots ? 1 : 0.45, textAlign: 'center' }}
                  onMouseOver={e => { if (hasSlots) e.currentTarget.style.borderColor = colors.accent; }}
                  onMouseOut={e => { if (hasSlots) e.currentTarget.style.borderColor = 'rgba(59,72,49,.15)'; }}
                >
                  <p style={{ fontSize: '14px', fontWeight: 600, color: hasSlots ? colors.primary : '#bbb', marginBottom: '4px' }}>{formatDate(date)}</p>
                  <p style={{ fontSize: '11px', color: hasSlots ? colors.accent : '#ccc' }}>{hasSlots ? `${slotCount} time${slotCount > 1 ? 's' : ''} available` : 'Unavailable'}</p>
                </button>
              );
            })}
          </div>
        ...
        {!availabilityLoading && Object.values(availabilityMap).every(d => !d.hasAvailability) && stayDates.length > 0 && (
          <p style={{ fontSize: '13px', color: colors.accent, marginTop: '16px', textAlign: 'center' }}>
            No availability for this treatment during your stay. Please call us at (903) 810-6695.
          </p>
        )}
      </div>
    );
  ...
  if (step === STEPS.TIME) {
    return (
      <div style={cardStyle}>
        ...
        <p style={subheadStyle}>{selectedService?.name} · {formatDate(selectedDate)}{selectedProvider ? ` · ${selectedProvider.name}` : ''}</p>
        {bookingError && (
          <div style={{ backgroundColor: 'rgba(197,124,93,.08)', border: `1px solid ${colors.accent}`, borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', fontSize: '13px', color: colors.accent }}>{bookingError}</div>
        )}
        {timeSlotsForDate.length > 0 ? (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            {timeSlotsForDate.map(slot => {
              ...
              return (
                <button key={slot} onClick={() => { if (conflict) { setBookingError('This therapist is already booked at this time.'); return; } handleSelectTime(slot); }} disabled={conflict}
                  style={{ ...pillBtnBase, backgroundColor: conflict ? 'rgba(0,0,0,.04)' : isSelected ? colors.accent : colors.card, color: conflict ? '#bbb' : isSelected ? '#fff' : colors.primary, border: conflict ? '1px solid rgba(0,0,0,.06)' : isSelected ? `2px solid ${colors.accent}` : `1px solid rgba(59,72,49,.15)`, fontWeight: isSelected ? 700 : 500, cursor: conflict ? 'not-allowed' : 'pointer', opacity: conflict ? 0.5 : 1 }}
          ...
          <p style={{ fontSize: '13px', color: '#8B7355', textAlign: 'center', padding: '20px' }}>No times available for this date. Please select another date.</p>
        )}
      </div>
    );
  }
  ...
  if (step === STEPS.CONFIRM) {
    return (
      <div style={cardStyle}>
        ...
        <button onClick={handleConfirmBooking} style={primaryBtn}>Confirm Selection</button>
      </div>
    );
  }
      ...
      <div>
        <div style={cardStyle}>
          <div style={{ textAlign: 'center', marginBottom: '20px' }}>
            <div style={{ width: '56px', height: '56px', borderRadius: '50%', backgroundColor: 'rgba(59,72,49,.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px', fontSize: '24px' }}>✓</div>
            <h2 style={{ ...headingStyle, marginBottom: '8px' }}>Treatment Selected!</h2>
            <p style={{ fontSize: '13px', color: '#8B7355' }}>
              {lastBooking?.serviceName}{lastBooking?.guestName ? ` for ${lastBooking.guestName}` : ''} on {formatDate(lastBooking?.date)} at {formatTime(lastBooking?.startTime)}{lastBooking?.providerName ? ` with ${lastBooking.providerName}` : ''}
            </p>
          </div>
          {completedBookings.length > 1 && (
            <div style={{ borderTop: '1px solid rgba(59,72,49,.08)', paddingTop: '16px', marginBottom: '16px' }}>
              <p style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: colors.primary, marginBottom: '12px' }}>All Selected Treatments</p>
              {completedBookings.map((b, i) => (
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={handleAddAnother} style={secondaryBtn}>+ Add Another Treatment</button>
          <button onClick={handleFinish} style={primaryBtn}>Continue</button>
        </div>
      </div>
    );
  }
