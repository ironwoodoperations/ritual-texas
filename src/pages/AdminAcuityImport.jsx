...
export default function AdminAcuityImport() {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        base44.auth.redirectToLogin(createPageUrl('AdminAcuityImport'));
      }
    };
    loadUser();
  }, []);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await base44.functions.invoke('importAcuityClients', {
    ...
    <div className="min-h-screen bg-[rgb(248,246,242)]">
      <header className="bg-white border-b border-[rgb(235,225,213)] px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-light text-[rgb(107,85,64)]">Import Acuity Clients</h1>
            <p className="text-sm text-[rgb(45,45,45)]">Upload CSV or Excel file from Acuity</p>
          </div>
        </div>
      </header>
