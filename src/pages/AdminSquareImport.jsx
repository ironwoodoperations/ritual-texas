...
export default function AdminSquareImport() {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        base44.auth.redirectToLogin(createPageUrl('AdminSquareImport'));
      }
    };
    loadUser();
  }, []);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await base44.functions.invoke('importSquareCustomers', {
    ...
    <div className="min-h-screen bg-[rgb(248,246,242)]">
      <header className="bg-white border-b border-[rgb(235,225,213)] px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-light text-[rgb(107,85,64)]">Import Square Customers</h1>
            <p className="text-sm text-[rgb(45,45,45)]">Upload CSV or Excel file from Square</p>
          </div>
        </div>
      </header>
