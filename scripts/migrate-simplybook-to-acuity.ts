// migrate-simplybook-to-acuity.ts
// One-time migration: create appointments in Acuity Scheduling from SimplyBook bookings.
//
// Usage:
//   ACUITY_USER_ID=xxx ACUITY_API_KEY=xxx deno run --allow-net --allow-env scripts/migrate-simplybook-to-acuity.ts
...
const AUTH_HEADER = "Basic " + btoa(`${ACUITY_USER_ID}:${ACUITY_API_KEY}`);

async function acuityGet(path: string) {
...
async function acuityPost(path: string, body: Record<string, unknown>) {
  const resp = await fetch(`${BASE_URL}${path}`, {
    method: "POST",
    headers: {
...
async function main() {
  ...
  console.log("\n=== Appointment Types ===");
  const types: { id: number; name: string }[] = await acuityGet("/appointment-types");
  ...
  // 2. Fetch calendars
  console.log("\n=== Calendars ===");
  const calendars: { id: number; name: string }[] = await acuityGet("/calendars");
